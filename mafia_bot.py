...
MAX_PLAYERS = 40; DEFAULT_NIGHT = 30; DEFAULT_VOTE = 30
...
GAME_IMAGE = None  # Will be set via file_id
CARD_NUMBER = "4073-4200-7154-7032"; ADMIN_ID = 7820231987
...
def load_profiles():
    try:
        with open(PROFILES_FILE, encoding="utf-8") as f: d = json.load(f)
    except: return {}
    changed = False
    for k, v in d.items():
        if "items" not in v: v["items"] = {}; changed = True
        for it in ["shield","kill_protect","vote_protect","rifle","mask","fake_doc"]:
            if it in v["items"] and isinstance(v["items"][it], int):
                v["items"][it] = {"count": v["items"][it], "active": True}; changed = True
            if it not in v["items"]: v["items"][it] = {"count": 0, "active": True}; changed = True
        for fld in ["hero","hero_attack","hero_defense"]:
            if fld not in v: v[fld] = (False if fld == "hero" else 0); changed = True
        for fld in ["dollars","olmos","evro","games","wins","losses"]:
            if fld not in v: v[fld] = 0; changed = True
        if "bought_role" not in v: v["bought_role"] = None; changed = True
    if changed: save_profiles(d)
    return d
...
def get_profile(uid, name=None):
    d = load_profiles(); k = str(uid)
    if k not in d:
        d[k] = {"name": name or str(uid), "dollars": 0, "olmos": 0, "evro": 0,
        ...
        save_profiles(d)
    elif name and d[k]["name"] == str(uid): d[k]["name"] = name; save_profiles(d)
    return d[k]

def save_profile(uid, data):
    d = load_profiles(); d[str(uid)] = data; save_profiles(d)
...
def remove_item(uid, item, count=1):
    ...
    if k in d and d[k].get("items", {}).get(item, {}).get("count", 0) >= count:
        d[k]["items"][item]["count"] -= count; save_profiles(d); return True
    return False

def add_item(uid, item, count=1):
    d = load_profiles(); k = str(uid)
    if k in d:
        if item not in d[k]["items"]: d[k]["items"][item] = {"count": 0, "active": True}
        d[k]["items"][item]["count"] += count; save_profiles(d)

def toggle_item(uid, item):
    d = load_profiles(); k = str(uid)
    if k in d and item in d[k]["items"]:
        d[k]["items"][item]["active"] = not d[k]["items"][item]["active"]
        save_profiles(d); return d[k]["items"][item]["active"]
    return False

def add_olmos(uid, amount):
    d = load_profiles(); k = str(uid)
    if k in d: d[k]["olmos"] = d[k].get("olmos", 0) + amount; save_profiles(d)

def spend_olmos(uid, amount):
    d = load_profiles(); k = str(uid)
    if k in d and d[k].get("olmos", 0) >= amount:
        d[k]["olmos"] -= amount; save_profiles(d); return True
    return False
...
class Player:
    def __init__(self, user_id, first_name, username=None):
        self.user_id = user_id; self.first_name = first_name; self.username = username
        self.role = None; self.alive = True; self.lover = None; self.defended = False
        self.guard_target = None; self.blocked = False; self.team = "village"
        self.actions_used = {}; self.hero = False
    @property
    def display(self): return f"@{self.username}" if self.username else self.first_name

class MafiaGame:
    def __init__(self, chat_id, mode="classic"):
        self.chat_id = chat_id; self.mode = mode
        self.players: Dict[int, Player] = {}; self.phase = "registration"; self.day = 0
        self.votes: Dict[int, int] = {}; self.actions = {}; self.used_actions = {}
        self.action_ready = {}; self.maniac_present = False; self.mine_target = None
        self.sniper_targets = {}; self.watch_results = {}; self.chemist_target = None
        self.fan_targets = {}; self.chameleon_targets = {}; self.drunk_targets = {}
        self.teacher_targets = {}; self.seller_targets = {}; self.blocked_players = set()
        self.rifle_uses: Dict[int, int] = {}; self.zombi_game = False; self.zombies = set()
        self.don_target = None; self.mafia_targets = {}; self.doc_choice = None
        self.maniac_target = None; self.advokat_target = None; self.msg_id = None; self.game_msg_id = None; self.start_time = None
    @property
    def alive_players(self): return [p for p in self.players.values() if p.alive]
    @property
    def mafia_alive(self): return [p for p in self.players.values() if p.role in ("Don", "Mafia") and p.alive]
...
async def update_game_msg(context, game):
    if not game.game_msg_id: return
...
def role_counts(game):
    counts = {}
    for p in game.alive_players: counts[p.role] = counts.get(p.role, 0) + 1
    return ", ".join(f"{ROLE_ICON.get(r,'')} {ROLE_DISPLAY.get(r,r)}" + (f" - {c}" if c > 1 else "") for r, c in sorted(counts.items()))

def make_kb(player_ids, action, icons=False, emoji=None):
    kb = []; row = []; lookup = {}; roles_d = {}
    for g in games.values():
        for pid, p in g.players.items(): lookup[pid] = p.display; roles_d[pid] = p.role
    for pid in player_ids:
        prefix = (emoji + " ") if emoji else ""
        name = lookup.get(pid, "?")
        if icons and pid in roles_d: name = f"{ROLE_ICON.get(roles_d[pid],'')} {name}"
...
def load_stats():
    try:
        with open(STATS_FILE, encoding="utf-8") as f: return json.load(f)
    except: return {}
...
def load_weekly():
    try:
        with open(WEEKLY_FILE, encoding="utf-8") as f: d = json.load(f)
    except: return {"week": int(time.time()), "players": {}, "distributed": False}
    now = int(time.time())
    week_sec = 7 * 24 * 3600
...
def save_weekly(data):
    with open(WEEKLY_FILE, "w", encoding="utf-8") as f: json.dump(data, f, indent=2, ensure_ascii=False)

def dist_weekly_prizes(w):
    if not w or not w.get("players"): return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (f"👋 Assalomu alaykum {u.first_name}! Mafia o'yin botiga xush kelibsiz.\n\n"
            ...
            "/help - 📖 Yordam")
    if GAME_IMAGE:
        await context.bot.send_photo(u.id, GAME_IMAGE, caption=text)
    else:
...
async def mafia(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("➕ O'yinga qo'shilish", callback_data="joingame")]])
        if GAME_IMAGE:
            msg = await context.bot.send_photo(chat_id, GAME_IMAGE, caption=text, reply_markup=kb)
        else:
...
async def join(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id not in games or games[chat_id].phase != "registration":
        await update.message.reply_text("Hozir qo'shilish mumkin emas!"); return
    game = games[chat_id]
    if user.id in game.players:
        await update.message.reply_text("Siz allaqachon o'yindasiz!"); return
    if len(game.players) >= MAX_PLAYERS:
        await update.message.reply_text("O'yin to'liq!"); return
    game.players[user.id] = Player(user.id, user.first_name, user.username)
    if chat_id not in ghosts: ghosts[chat_id] = set()
...
async def leave(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    if chat_id not in games or games[chat_id].phase != "registration":
        await update.message.reply_text("Hozir chiqib bo'lmaydi!"); return
    game = games[chat_id]
    if user.id not in game.players:
        await update.message.reply_text("Siz o'yinda emassiz!"); return
    del game.players[user.id]
    if chat_id in ghosts and user.id in ghosts[chat_id]: ghosts[chat_id].discard(user.id)
    text = f"O'yinchilar ({len(game.players)}/{MAX_PLAYERS}):\n"
    for i, p in enumerate(game.players.values(), 1):
        text += f"{i}. {p.display}\n"
    await update.message.reply_text(text)
    if not game.players:
        del games[chat_id]; del ghosts[chat_id]
        await update.message.reply_text("Hech kim qolmadi, o'yin bekor qilindi.")

async def players(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    if chat_id not in games:
        await update.message.reply_text("O'yin yo'q!"); return
    game = games[chat_id]
    if not game.players:
        await update.message.reply_text("O'yinchilar yo'q!"); return
...
async def money(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    prof = get_profile(user.id, user.first_name)
    await update.message.reply_text(f"💰 Hisob:\n💎 Olmos: {prof['olmos']}\n💵 Dollar: {prof['dollars']}\n💶 Evro: {prof['evro']}")

async def send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    text = update.message.text; parts = text.split()
    if len(parts) < 3:
        await update.message.reply_text("Format: /send @username sum"); return
    target_name = parts[1].strip("@")
    try: amount = int(parts[2])
    except: await update.message.reply_text("Noto'g'ri summa!"); return
    if amount <= 0: await update.message.reply_text("Summa musbat!"); return
    prof = get_profile(user.id)
    if prof["dollars"] < amount:
        await update.message.reply_text("Yetarli dollar yo'q!"); return
    target_id = None
    for g in games.values():
        ...
        if target_id: break
    if not target_id:
        await update.message.reply_text("Foydalanuvchi topilmadi!"); return
    prof["dollars"] -= amount; save_profile(user.id, prof)
    tprof = get_profile(target_id)
    tprof["dollars"] = tprof.get("dollars", 0) + amount; save_profile(target_id, tprof)
...
async def give(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = update.message.text; parts = text.split()
    if len(parts) < 3:
        await update.message.reply_text("Format: /give @username sum"); return
    target_name = parts[1].strip("@")
    try: amount = int(parts[2])
    except: await update.message.reply_text("Noto'g'ri summa!"); return
    if amount <= 0: return
...
async def gsend(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = update.message.text; parts = text.split()
    if len(parts) < 4:
        await update.message.reply_text("Format: /gsend @username miqdor tur"); return
    target_name = parts[1].strip("@")
    try: amount = int(parts[2])
    except: await update.message.reply_text("Noto'g'ri summa!"); return
    currency = parts[3].lower()
    if currency not in ("dollar", "olmos", "evro"):
        await update.message.reply_text("dollar/olmos/evro"); return
    target_id = None
    for g in games.values():
        for pid, p in g.players.items():
            if p.username and p.username.lower() == target_name.lower():
    ...
    prof[currency] = prof.get(currency, 0) + amount; save_profile(target_id, prof)
...
async def change(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = update.message.text; parts = text.split()
    if len(parts) < 2:
        await update.message.reply_text("Format: /change summa"); return
    try: amount = int(parts[1])
    except: await update.message.reply_text("Noto'g'ri summa!"); return
    if amount <= 0: await update.message.reply_text("Summa musbat!"); return
    prof = get_profile(user.id)
    if prof["olmos"] < amount:
        await update.message.reply_text("Yetarli olmos yo'q!"); return
    prof["olmos"] -= amount; prof["evro"] = prof.get("evro", 0) + amount
...
async def giveaway(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 1:
        await update.message.reply_text("Format: /giveaway sum"); return
...
async def startgame(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_admin(update.effective_chat, user.id, context):
        await update.message.reply_text("Faqat admin!"); return
    if chat_id not in games or games[chat_id].phase != "registration":
        await update.message.reply_text("O'yin mavjud emas!"); return
    game = games[chat_id]
    if len(game.players) < get_set(chat_id)["min"]:
        await update.message.reply_text(f"Kamida {get_set(chat_id)['min']} o'yinchi kerak!"); return
    await start_gameplay(update, context, game)

async def start_gameplay(update, context, game):
    chat_id = game.chat_id
    players = list(game.players.values())
    count = len(players); mode = game.mode
    ...
    if count <= 6:
        ...
        assigned = base_roles[:]
        while len(assigned) < count: assigned.append("Tinch aholi")
        assigned = assigned[:count]
    random.shuffle(assigned)
    for p, role in zip(players, assigned):
        p.role = role
        if role in ("Don", "Mafia"): p.team = "mafia"
        elif role in ("Manyak", "Ubica", "Zombi"): p.team = "neutral"
        else: p.team = "village"
    game.day = 0; game.phase = "night"; game.start_time = time.time()
    msg = await context.bot.send_animation(chat_id, NIGHT_GIF, caption=f"🌙 *{count} o'yinchi bilan o'yin boshlandi!*\n1-tun boshlanishi...", parse_mode="Markdown")
    for p in players:
        try:
...
async def night_phase(context, game):
    chat_id = game.chat_id
    game.phase = "night"; game.day += 1
    game.actions = {}; game.votes = {};
    game.don_target = None; game.mafia_targets = {}; game.doc_choice = None
    game.maniac_target = None; game.advokat_target = None; game.mine_target = None
    game.sniper_targets = {}; game.chemist_target = None; game.fan_targets = {}
    game.chameleon_targets = {}; game.drunk_targets = {}; game.teacher_targets = {}
    game.seller_targets = {}; game.blocked_players = set()
    alive = game.alive_players
    if len(alive) < 2:
        await end_game(context, game); return
    for p in alive:
...
async def send_night_actions(context, game, p):
    ...
    if role == "Komissar":
        ...
        kb = make_kb(alive_ids, f"ndon_kill:{game.day}", emoji="🩸")
    elif role == "Mafia":
        text += "Kimni o'ldirishga ovoz berasiz? 🩸"
        kb = make_kb(alive_ids, f"nmafia_vote:{game.day}", emoji="🩸")
    elif role == "Serjant":
        text += "Kimni himoya qilishga ovoz berasiz? 🗳"
        kb = make_kb(alive_ids, f"nserjant:{game.day}", emoji="🗳")
    elif role == "Shifokor":
        text += "Kimni davolaysiz? 💊"
        kb = make_kb(alive_ids, f"ndoc:{game.day}", emoji="💊")
    elif role == "Manyak":
        text += "Kimni o'ldirasiz? 🗡"
        kb = make_kb(alive_ids, f"nmaniac:{game.day}", emoji="🗡")
    elif role == "Daydi":
        text += "Kimga tashrif buyurasiz? 👤"
        kb = make_kb(alive_ids, f"ndaydi:{game.day}")
    elif role == "Advokat":
        text += "Kimni himoya qilasiz? ⚖"
        kb = make_kb(alive_ids, f"nadv:{game.day}", emoji="⚖")
    elif role == "Bodyguard":
        text += "Kimni qo'riqlaysiz? 🛡"
        kb = make_kb(alive_ids, f"nguard:{game.day}", emoji="🛡")
    elif role == "Oshiq":
        text += "Sevganingizni tanlang: 💕"
        kb = make_kb(alive_ids, f"noshik:{game.day}", emoji="💕")
    elif role == "Mashuqa":
        text += "Oshigingiz bilan muloqot: 💕"
        kb = make_kb(alive_ids, f"nmashuqa:{game.day}", emoji="💕")
    elif role == "Aferist":
        text += "Kimni bloklaysiz? 🔒"
        kb = make_kb(alive_ids, f"nafer:{game.day}", emoji="🔒")
    elif role == "Sehrgar":
        text += "Sehrli kuchingizni kimga ishlatasiz? ✨"
        kb = make_kb(alive_ids, f"nsehr:{game.day}", emoji="✨")
    elif role == "Don xotini":
        text += "Kimni tekshirasiz? 🔍"
        kb = make_kb(alive_ids, f"ndonx:{game.day}", emoji="🔍")
    elif role == "Kimyogar":
        text += "Kimga zahar tayyorlaysiz? ☠"
        kb = make_kb(alive_ids, f"nkimyo:{game.day}", emoji="☠")
    elif role == "Sotuvchi":
        text += "Kimga qurol sotasiz? 🔫"
        kb = make_kb(alive_ids, f"nsotuv:{game.day}", emoji="🔫")
    elif role == "Tentak":
        text += "Kimga yopishasiz? 🐙"
        kb = make_kb(alive_ids, f"ntentak:{game.day}", emoji="🐙")
    elif role == "Oqituvchi":
        text += "Kimga dars berasiz? 📚"
        kb = make_kb(alive_ids, f"noqit:{game.day}", emoji="📚")
    elif role == "Muxlis":
        text += "Kumingizni tanlang: 👁"
        kb = make_kb(alive_ids, f"nmuxlis:{game.day}", emoji="👁")
    elif role == "Mergan":
        text += "Kimni otasiz? 🎯"
        kb = make_kb(alive_ids, f"nmergan:{game.day}", emoji="🎯")
    elif role == "Majnun":
        text += "Kimga bog'lanasiz? 🔗"
        kb = make_kb(alive_ids, f"nmajnun:{game.day}", emoji="🔗")
    elif role == "Ubica":
        text += "Kimni o'ldirasiz? 💀"
        kb = make_kb(alive_ids, f"nubica:{game.day}", emoji="💀")
...
async def resolve_night(context, game):
    chat_id = game.chat_id
    kills = set(); protected = set(); doc_save = set()
    daydi_visitors = {}
    blocked_players = set()
    
    for uid, action in game.actions.items():
        player = game.get_player(uid)
        if not player or not player.alive: continue
        target_id = action.get("target"); atype = action.get("type", "")
        
        if atype == "don_kill" and target_id: game.don_target = target_id
        elif atype == "mafia_vote" and target_id: game.mafia_targets[uid] = target_id
        elif atype == "doc_heal" and target_id: game.doc_choice = target_id
        elif atype == "kom_check" and target_id:
            if game.day > 1:
                tp = game.get_player(target_id)
                if tp:
                    res = "Mafiya" if tp.team in ("mafia", "neutral") else "Fuqaro"
                    try: await context.bot.send_message(uid, f"🔍 Natija: {res}")
                    except: pass
        elif atype == "kom_kill" and target_id: kills.add(target_id)
        elif atype == "maniac_kill" and target_id: game.maniac_target = target_id
        elif atype == "adv_protect" and target_id: game.advokat_target = target_id
        elif atype == "guard_protect" and target_id: protected.add(target_id)
        elif atype == "serjant_vote" and target_id: game.mafia_targets[uid] = target_id
        elif atype == "daydi_visit" and target_id:
            if target_id not in daydi_visitors: daydi_visitors[target_id] = []
            daydi_visitors[target_id].append(uid)
        elif atype == "afer_blok" and target_id: blocked_players.add(target_id)
        elif atype == "kimyo_poison" and target_id: kills.add(target_id)
        elif atype == "mergan_shoot" and target_id: kills.add(target_id)
        elif atype == "ubica_kill" and target_id: kills.add(target_id)
        elif atype == "sehr_magic" and target_id: blocked_players.add(target_id)
        elif atype == "tentak_stick" and target_id: blocked_players.add(target_id)
        elif atype == "sotuv_sell" and target_id: add_item(target_id, "rifle", 1)
            ...
            game.fan_targets[uid] = target_id
        elif atype == "majnun_bond" and target_id:
            game.fan_targets[uid] = target_id
        elif atype == "oqit_teach" and target_id:
            try: await context.bot.send_message(target_id, "📚 O'qituvchi sizga dars berdi! Keyingi ovozingiz 2 hisoblanadi.")
            except: pass
    ...
    # Send Daydi visitor list
    for target_id, visitors in daydi_visitors.items():
        for daydi_uid in visitors:
            names = []
            for v_uid in visitors:
                vp = game.get_player(v_uid)
                if vp and v_uid != daydi_uid: names.append(vp.display)
            if names:
                try: await context.bot.send_message(daydi_uid, f"👤 Sizga tashrif buyurganlar: {', '.join(names)}")
                except: pass
    
    # Serjant replaces Komissar
    if not game.get_player(ADMIN_ID) or True:  # Check if Komissar is dead
        komissar_dead = True
        serjant_alive = None
        for p in game.players.values():
            if p.role == "Komissar" and p.alive: komissar_dead = False
            if p.role == "Serjant" and p.alive: serjant_alive = p
        if komissar_dead and serjant_alive and serjant_alive.role == "Serjant":
            serjant_alive.role = "Komissar"
            serjant_alive.team = "village"
            try: await context.bot.send_message(serjant_alive.user_id, "🕵️‍♂️ Komissar o'ldi! Endi siz yangi Komissarsiz!")
            except: pass
    
    # Mafia kill voting
    if game.mafia_targets:
        from collections import Counter
        c = Counter(game.mafia_targets.values())
        if c:
            mc = c.most_common(1)[0]
            if mc[1] >= 2: kills.add(mc[0])
            elif game.don_target: kills.add(game.don_target)
            elif mc: kills.add(mc[0])
    if not kills and game.don_target: kills.add(game.don_target)
    if game.maniac_target: kills.add(game.maniac_target)
    
    killed_players = []; saved_by_doc = False; saved_by_adv = False
    for tid in kills:
        if tid in protected or game.doc_choice == tid:
            if game.doc_choice == tid: saved_by_doc = True
            if tid in protected: saved_by_adv = True
            continue
        if game.advokat_target == tid:
            saved_by_adv = True
            continue
        if has_item(tid, "shield") and remove_item(tid, "shield"):
            try: await context.bot.send_message(tid, "🛡 Himoya qutqardi!")
            except: pass
            continue
        ...
        if target and target.alive:
            ...
            if prof.get("hero") and random.random() < prof.get("hero_defense", 0) * 0.01:
                try: await context.bot.send_message(tid, "🦸 Qahramon qutqardi!")
                except: pass
                continue
            if target.role == "Omadli" and random.random() < 0.5:
                try: await context.bot.send_message(tid, "🍀 Omad qutqardi!")
                except: pass
                continue
            target.alive = False; killed_players.append(target)
    
    if killed_players:
        names = ", ".join(fmt_player(p) for p in killed_players)
        await context.bot.send_message(chat_id, f"☠ O'ldirildi:\n{names}", parse_mode="HTML")
    else:
        msg = []
        if saved_by_doc: msg.append("🏥 Shifokor bir kishini o'limdan qutqardi!")
        if saved_by_adv: msg.append("⚖ Advokat bir kishini himoya qildi!")
        if not msg: msg.append("☀ Bugun hech kim o'lmadi.")
        await context.bot.send_message(chat_id, "\n".join(msg))
    
    await context.bot.send_animation(chat_id, DAY_GIF, caption=f"☀ {game.day}-kun boshlandi!")
    game.phase = "day"
...
async def check_win(context, game):
    ...
    neutral_killers = [p for p in alive if p.role in ("Manyak", "Ubica", "Minior")]
    if not mafia and not neutral_killers:
        await end_game(context, game, "village")
    elif not village and not neutral_killers:
        await end_game(context, game, "mafia")
    elif len(mafia) >= len(village):
        await end_game(context, game, "mafia")
    else:
...
async def day_phase(context, game):
    ...
    for p in game.players.values():
        r = p.role
        if r: role_list[r] = role_list.get(r, 0) + 1
    ...
    await context.bot.send_message(chat_id, text, reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
...
async def resolve_vote(context, game):
    chat_id = game.chat_id
    if not game.votes:
        await context.bot.send_message(chat_id, "Hech kim ovoz bermadi."); await night_phase(context, game); return
    from collections import Counter
    c = Counter(game.votes.values())
    if not c:
        await context.bot.send_message(chat_id, "Hech kim ovoz bermadi."); await night_phase(context, game); return
    mc = c.most_common(1)[0]
    max_v = mc[1]; alive = game.alive_players
    half = len(alive) / 2
    candidates = [uid for uid, v in c.items() if v == max_v]
    if max_v <= half or len(candidates) > 1:
        await context.bot.send_message(chat_id, f"Hech kim chiqarilmadi ({max_v}/{len(alive)}).")
        await night_phase(context, game); return
    ejected_id = candidates[0]; ejected = game.get_player(ejected_id)
    if ejected and ejected.alive:
        if has_item(ejected_id, "vote_protect") and remove_item(ejected_id, "vote_protect"):
            await context.bot.send_message(chat_id, f"🛡 {ejected.display} ovoz himoyasi bilan qutuldi!")
            await night_phase(context, game); return
        
        # Suidsid o'zini o'ldiradi
        if ejected.role == "Suidsid":
            ejected.alive = False
            await context.bot.send_message(chat_id, f"💔 {ejected.display} (Suitsid) ovoz berilishidan oldin o'zini o'ldirdi!")
            rd = f"{ROLE_ICON.get(ejected.role,'')} {ROLE_DISPLAY.get(ejected.role,ejected.role)}"
            await context.bot.send_message(chat_id, f"{ejected.display} o'zini o'ldirdi! Rol: {rd}", parse_mode="HTML")
            await check_win(context, game); return
        
        ejected.alive = False
        if ejected.role == "Kamikaze":
            await context.bot.send_message(chat_id, "💥 Kamikaze portladi!")
            for vid, tid in game.votes.items():
                if tid == ejected_id:
                    vp = game.get_player(vid)
                    if vp and vp.alive: vp.alive = False; await context.bot.send_message(chat_id, f"💥 {vp.display} portlashda o'ldi!")
                    break
        elif ejected.role == "Minior":
            await context.bot.send_message(chat_id, "💥 Minior portladi!")
            lst = [p for p in game.alive_players if p.user_id != ejected_id]
            if lst: t = random.choice(lst); t.alive = False; await context.bot.send_message(chat_id, f"💥 {t.display} portlashda o'ldi!")
        if has_item(ejected_id, "mask") and remove_item(ejected_id, "mask"):
            await context.bot.send_message(chat_id, f"🎭 {ejected.display}ning roli noma'lum!")
        else:
            rd = f"{ROLE_ICON.get(ejected.role,'')} {ROLE_DISPLAY.get(ejected.role,ejected.role)}"
            await context.bot.send_message(chat_id, f"{ejected.display} chiqarildi! Rol: {rd}", parse_mode="HTML")
    await check_win(context, game)
...
async def end_game(context, game, winner=None):
    ...
    if winner == "village":
        ...
        title = "O'yin tugadi! 🎮"
        winners = []; losers = list(game.players.values())
    ...
    for p in game.players.values():
        ...
        if winner == "village" and p.team == "village":
            prof["wins"] = prof.get("wins", 0) + 1; prof["dollars"] = prof.get("dollars", 0) + 10
        elif winner == "mafia" and p.team == "mafia":
            prof["wins"] = prof.get("wins", 0) + 1; prof["dollars"] = prof.get("dollars", 0) + 15
        else: prof["losses"] = prof.get("losses", 0) + 1
        save_profile(p.user_id, prof)
        uid_str = str(p.user_id)
        if uid_str not in w["players"]: w["players"][uid_str] = {"score": 0}
        was_winner = (winner == "village" and p.team == "village") or (winner == "mafia" and p.team == "mafia")
        w["players"][uid_str]["score"] = w["players"][uid_str].get("score", 0) + (3 if was_winner else 1)
    save_weekly(w)
    del games[chat_id]
    if chat_id in ghosts: del ghosts[chat_id]
...
async def vote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id not in games or games[chat_id].phase != "day":
        await update.message.reply_text("Hozir ovoz vaqti emas!"); return
    game = games[chat_id]
    if user.id not in game.players or not game.players[user.id].alive:
        await update.message.reply_text("Siz tirik emassiz!"); return
    args = context.args
    if not args: await update.message.reply_text("Format: /vote @user"); return
    target_name = args[0].strip("@")
    target_id = None
    for pid, p in game.players.items():
        if p.alive and p.username and p.username.lower() == target_name.lower():
            target_id = pid; break
    if not target_id: await update.message.reply_text("Topilmadi!"); return
...
async def defend(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id; user = update.effective_user
    if chat_id not in games or games[chat_id].phase != "day":
        await update.message.reply_text("Hozir himoya vaqti emas!"); return
    game = games[chat_id]
    if user.id not in game.players or not game.players[user.id].alive:
        await update.message.reply_text("Siz tirik emassiz!"); return
...
async def status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    if chat_id not in games: await update.message.reply_text("O'yin yo'q!"); return
    game = games[chat_id]; alive = game.alive_players
    ...
    for p in alive: text += f"• {p.display}\n"
    if dead:
        text += f"\nO'lgan ({len(dead)}):\n"
        for p in dead: text += f"• {p.display}\n"
    await update.message.reply_text(text)

async def ghost(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id; user = update.effective_user
    if chat_id not in ghosts or user.id not in ghosts[chat_id]:
        await update.message.reply_text("Ghost chatga kira olmaysiz!"); return
    msg = update.message.text
    if not msg.startswith("/g "): return
    text = msg[3:].strip()
    if not text: return
...
async def top(update: Update, context: ContextTypes.DEFAULT_TYPE):
    s = load_stats(); profs = load_profiles()
...
async def set_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if len(args) < 2:
        await update.message.reply_text("Format: /set param qiymat"); return
    param = args[0].lower(); value = args[1]
    setts = get_set(chat_id)
    if param == "min":
        try: v = int(value)
        except: await update.message.reply_text("Noto'g'ri!"); return
        if 1 <= v <= MAX_PLAYERS: setts["min"] = v; await update.message.reply_text(f"Min: {v}")
        else: await update.message.reply_text(f"1-{MAX_PLAYERS}")
    elif param == "night":
        try: v = int(value)
        except: await update.message.reply_text("Noto'g'ri!"); return
        if 5 <= v <= 120: setts["night"] = v; await update.message.reply_text(f"Tun: {v}s")
        else: await update.message.reply_text("5-120")
    elif param == "vote":
        try: v = int(value)
        except: await update.message.reply_text("Noto'g'ri!"); return
        if 5 <= v <= 120: setts["vote"] = v; await update.message.reply_text(f"Ovoz: {v}s")
        else: await update.message.reply_text("5-120")
    elif param == "mode":
        if value.lower() in GAME_MODES:
            setts["mode"] = value.lower()
            await update.message.reply_text(f"Mode: {GAME_MODES[value.lower()]}")
            if chat_id in games: games[chat_id].mode = value.lower()
        else: await update.message.reply_text("classic/full")
    else: await update.message.reply_text("Param: min, night, vote, mode")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "joingame":
        ...
        if chat_id not in games or games[chat_id].phase != "registration":
            await query.answer("O'yin mavjud emas yoki ro'yxat tugagan!", show_alert=True); return
        game = games[chat_id]
        if user_id in game.players:
            await query.answer("Siz allaqachon o'yindasiz!", show_alert=True); return
        if len(game.players) >= MAX_PLAYERS:
            await query.answer("O'yin to'liq!", show_alert=True); return
    ...
    if data == "cancel_payment":
        await query.edit_message_text("Bekor qilindi."); return
    if data.startswith("confirm_pay:"):
        if user_id != ADMIN_ID: await query.answer("Admin emas!", show_alert=True); return
        parts = data.split(":")
        if len(parts) >= 3:
            tuid = int(parts[1]); amt = int(parts[2])
            add_olmos(tuid, amt)
            await query.edit_message_text(f"✅ {amt} olmos tasdiqlandi!")
            try: await context.bot.send_message(tuid, f"✅ {amt} olmos hisobga tushdi!")
            except: pass
        return
    if data.startswith("reject_pay:"):
        ...
        if len(parts) >= 2:
            ...
            try: await context.bot.send_message(tuid, "❌ To'lov rad etildi.")
            except: pass
        return
    ...
    if data == "buyhero":
        ...
        if prof.get("hero"): await query.edit_message_text("Hero bor!"); return
        if prof["olmos"] < 90: await query.edit_message_text("90 olmos kerak!"); return
        prof["olmos"] -= 90; prof["hero"] = True
        prof["hero_attack"] = random.randint(5, 15); prof["hero_defense"] = random.randint(5, 15)
    ...
    if data.startswith("buyrole:"):
        role = data.split(":", 1)[1]; prof = get_profile(user_id)
        price = ROLE_PRICES.get(role, 0)
        if prof["olmos"] < price: await query.edit_message_text(f"{price} olmos kerak!"); return
        prof["olmos"] -= price; prof["bought_role"] = role; save_profile(user_id, prof)
        await query.edit_message_text(f"✅ {ROLE_ICON.get(role,'')} {ROLE_DISPLAY.get(role,role)} sotib olindi!")
        return

    # Shop
    if data.startswith("buy:"):
        item_id = data.split(":", 1)[1]; prof = get_profile(user_id)
        price = ITEM_PRICES.get(item_id, 0)
        if prof["olmos"] < price: await query.edit_message_text(f"{price} olmos kerak!"); return
        prof["olmos"] -= price
        if item_id not in prof["items"]: prof["items"][item_id] = {"count": 0, "active": True}
        prof["items"][item_id]["count"] += 1; save_profile(user_id, prof)
        await query.edit_message_text(f"✅ {ITEM_NAMES.get(item_id, item_id)} (x{prof['items'][item_id]['count']})")
        return

    # Komissar multi-action: choose action first
    if data.startswith("nact:"):
        parts = data.split(":")
        if len(parts) < 3: return
        action_prefix = parts[1]; day = parts[2]
        game = find_game(user_id)
        if not game or game.phase != "night": await query.edit_message_text("Harakat vaqti emas!"); return
        ...
        kb = make_kb(alive_ids, f"ntarget:{action_prefix}", emoji=em)
        text = f"🌙 {game.day}-tun. " + ("Kimni tekshirasiz? 🔍" if "check" in action_prefix else "Kimni o'ldirasiz? 🗡")
        await query.edit_message_text(text, reply_markup=kb)
        return

    # Target selection for multi-action
    if data.startswith("ntarget:"):
        parts = data.split(":")
        if len(parts) < 3: return
        action_prefix = parts[1]; target_id = int(parts[2])
        game = find_game(user_id)
        if not game or game.phase != "night": await query.edit_message_text("Harakat vaqti emas!"); return
        if user_id in game.actions: await query.edit_message_text("Allaqachon harakat qilgansiz!"); return
        target_player = game.get_player(target_id)
        if not target_player or not target_player.alive: await query.edit_message_text("Tirik emas!"); return
        ...
        if game and user_id in game.players:
            ...
            try: await context.bot.send_message(game.chat_id, msg)
            except: pass
        emoji_map = {"kom_check": "🔍", "kom_kill": "🗡"}
        em = emoji_map.get(action_prefix, "")
        await query.edit_message_text(f"{em} {target_player.display} " + ("tekshirilmoqda..." if "check" in action_prefix else "o'ldirilmoqda..."))
        if user_id in night_step: del night_step[user_id]
        return

    # Single-step night actions
    ...
    if data.startswith(night_single):
        prefix = data.split(":")[0]; parts = data.split(":")
        if len(parts) < 3: await query.edit_message_text("Xatolik!"); return
        target_id = int(parts[2])
        game = find_game(user_id)
        if not game: await query.edit_message_text("O'yinda emassiz!"); return
        if game.phase != "night": await query.edit_message_text("Hozir tun emas!"); return
        p = game.get_player(user_id)
        if not p or not p.alive: await query.edit_message_text("Siz o'lgansiz!"); return
        if user_id in game.actions: await query.edit_message_text("Allaqachon harakat qilgansiz!"); return
        target_player = game.get_player(target_id)
        if not target_player or not target_player.alive: await query.edit_message_text("Tirik emas!"); return
        action_map = {
    ...
    if data.startswith("vote:"):
        ...
        if len(parts) >= 3:
            ...
            game = find_game(user_id)
            if not game: await query.edit_message_text("O'yinda emassiz!"); return
            if game.phase != "day": await query.edit_message_text("Ovoz vaqti emas!"); return
            p = game.get_player(user_id)
            if not p or not p.alive: await query.edit_message_text("Siz o'lgansiz!"); return
            tp = game.get_player(target_id)
            if not tp or not tp.alive: await query.edit_message_text("Tirik emas!"); return
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == ADMIN_ID:
        ...
        global GAME_IMAGE; GAME_IMAGE = photo.file_id
        cfg = load_config(); cfg["game_image"] = photo.file_id; save_config(cfg)
        await update.message.reply_text(f"✅ Rasm saqlandi!")
        return
    
    if user.id not in pending_checks or pending_checks[user.id].get("step") != "waiting_photo": return
    photo = update.message.photo[-1]
    ...
    await update.message.reply_text("✅ Chek qabul qilindi! Summani yozing (olmos):\nMisol: 100")
    ...
    if user.id in pending_checks:
        ...
        if state.get("step") == "waiting_amount":
            try:
                ...
                if amount <= 0 or amount > 10000: await update.message.reply_text("1-10000!"); return
                state["amount"] = amount; state["step"] = "confirmed"
            ...
            except: await update.message.reply_text("Son yozing! Masalan: 100")
...
def main():
    ...
    cmds = [
        ...
        ("help", help_cmd),("top", top),("hafta", hafta),("stats", stats),        ("settings", settings_cmd),("set", set_cmd),("setimage", setimage),
